            ...
            try
            {
                ...
                foreach (var limit in paramLimits)
                {
                    ...
                    if (currentValue < limit.MinAcceptValue)
                    {
                        ...
                        parameterEvaluations[limit.ParameterName] = $"Below acceptable range by {distance:F2}";
                    }
                    else if (currentValue > limit.MaxAcceptValue)
                    {
                        var distance = currentValue - limit.MaxAcceptValue;
                        parameterEvaluations[limit.ParameterName] = $"Above acceptable range by {distance:F2}";
                    }
                    else
                    {
